...
public class BookController {
	...
	public Book searchBookID(int bookID, int copyID) {
		...
		String sql = "SELECT Book.BookID,Copy.CopyID,title,TypeName,Author,COUNT(Copy.bookID),price, copyCondition FROM Book "
				+ "INNER JOIN booktype ON Book.typeID=booktype.typeID "
				+ "INNER JOIN Copy ON book.bookID=copy.bookID WHERE Book.BookID = ? AND Copy.CopyID=? ";
		...
		try {
			...
			while (rs.next()) {
				...
				book.setCondition(rs.getString(8));
			}
